...
function QuikLinks() {
  ...
  <div className=" mt-10 lg:mx-16 gap-2 sm:gap-5 mb-5 sm:mb-20 grid grid-cols-2 sm:grid-cols-2  md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 ">
    {carCards.map((card) => (
      <div
        key={card.id}
        className=" w-full rounded-[20px] p-5 text-white shadow-md flex flex-col justify-between"
        style={{
      ...
      >
        {/* <div
        key={card.id}
        className=" w-[175px] h-[170px] md:w-[95%] md:h-[220px] lg:w-[290px] lg:h-[290px]  xl:w-[250px] xl:h-[250px] rounded-[20px] p-5 text-white shadow-md flex flex-col justify-between"
        style={{
          background: "linear-gradient(to top right, #FF5722, #B52C00)",
        }}
      > */}
